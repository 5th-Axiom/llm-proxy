...
// AppState is the immutable snapshot of everything derived from a Config. A
// new AppState is built for every Reload and installed via atomic pointer
// swap; existing goroutines that already captured a pointer keep using the
...
// supplied http.Client for upstream calls. The client is shared across
// reloads so TLS sessions and idle connections survive config changes.
func NewContainer(client *http.Client) *Container {
...
func (c *Container) Install(raw config.Config) error {
	...
	state := &AppState{
		Registry:      registry,
		Authenticator: auth.New(resolved.Server.Tokens),
